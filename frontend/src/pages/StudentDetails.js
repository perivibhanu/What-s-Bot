...
function StudentDetails() {
  ...
  const fetchStudent = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/students/${id}`, {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/students/${id}`, editData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setEditMode(false);
      fetchStudent();
