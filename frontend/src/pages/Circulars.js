...
function Circulars() {
  ...
  const fetchCirculars = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/circulars`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/circulars`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setShowModal(false);
  ...
  const handleResend = async (id) => {
    if (!window.confirm('Are you sure you want to re-send this circular to all registered students?')) {
      return;
    }
