...
function WardenIssuesFeedback() {
  ...
  const fetchIssues = async () => {
    try {
      ...
      console.error('Failed to fetch warden issues:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Layout>
      <div className="students-header">
        <div>
          <h1>👨‍✈️ Warden Issues</h1>
          <p className="student-count">
            Manage complaints and requests from Hostel Wardens.
          </p>
        </div>
      </div>
        ...
        <table>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="8" style={{textAlign:'center'}}>No warden issues found.</td></tr>
            ) : (
              issues.map(issue => (
                <tr key={issue._id}>
                  <td style={{ fontWeight: 'bold' }}>{issue.ticketId}</td>
                  <td>{issue.wardenId?.name || 'Unknown'}</td>
                  <td>{issue.wardenId?.block || '-'}</td>
                  <td>{issue.wardenId?.mobileNumber || '-'}</td>
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '500px' }}>
            ...
            <p style={{ marginBottom: '1rem', color: '#475569' }}>
              ...
              <strong>Warden:</strong> {selectedIssue?.wardenId?.name}
            </p>
            ...
            <form onSubmit={handleUpdateStatus}>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <textarea 
                  ...
                  placeholder="Will be sent via WhatsApp to the warden..."
                />
              </div>
...
export default WardenIssuesFeedback;
