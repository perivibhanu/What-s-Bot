...
function Students() {
  ...
  const fetchBatchYears = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/students/batch-years`, {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/students/import/template`, {
        headers: { Authorization: `Bearer ${token}` },
        responseType: 'blob'
      });
  ...
  const handleBulkImport = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/students/import/bulk`, formData, {
        headers: {
  ...
  const handleBulkDelete = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.delete(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/students/bulk-delete/${deleteBatchYear}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
