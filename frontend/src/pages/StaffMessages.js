...
function StaffMessages() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/staff-messages', {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/staff-messages/upload', formDataUpload, {
        headers: { 
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/staff-messages', createFormData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSend = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`http://localhost:5000/api/staff-messages/${selectedMessageId}/send`, targetPayload, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleResend = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`http://localhost:5000/api/staff-messages/${id}/resend`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert(res.data.message);
      fetchMessages();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/staff-messages/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchMessages();
