...
function HodParentCirculars() {
  ...
  const fetchCirculars = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/circulars?type=parent_hod&department=${dept}`, {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/circulars/upload`, formDataUpload, {
        headers: { 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${process.env.REACT_APP_API_URL}/api/circulars`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setShowModal(false);
  ...
  const handleSend = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/circulars/${selectedCircularId}/send`, targetPayload, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleResend = async (id) => {
    if (!window.confirm(`Are you sure you want to re-send this circular to all registered ${dept} parents?`)) {
      return;
    }
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/circulars/${id}/resend`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert(res.data.message);
      fetchCirculars();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${process.env.REACT_APP_API_URL}/api/circulars/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchCirculars();
...
export default HodParentCirculars;
